...
def set_simulated_price_deltas(
    bid_delta: Optional[float] = None, ask_delta: Optional[float] = None
) -> None:
...
def _parse_delta_string(s: str) -> timedelta:
    ...
    if not m:
        raise ValueError(
            "Invalid delta string. Use '30s', '5min', '2h', '1d' or raw seconds."
        )
...
def set_global_delta(delta: Optional[Union[str, int, float, timedelta]]) -> None:
    ...
    if isinstance(delta, str):
        ...
        return
    raise ValueError(
        "Unsupported delta type. Provide timedelta, number (seconds), or string like '5min'."
    )
...
def _extract_pair_and_granularity(
    symbol_value: str, default_granularity: str = "1min"
) -> Tuple[str, str]:
...
class CustomEurUsdQuoteData(PythonData):
    ...
    def get_source(
        self, config: SubscriptionDataConfig, date: datetime, is_live_mode: bool
    ) -> SubscriptionDataSource:
        ...
        pair, gran = _extract_pair_and_granularity(
            sym_val or "EURUSD", default_granularity=default_gran
        )
    ...
    def reader(
        ...
        try:
            ...
            default_gran = _granularity_from_increment(
                getattr(config, "increment", None)
            )
            alias = getattr(config, "symbol", None)
            sym_val = getattr(alias, "value", str(alias))
            _pair, gran = _extract_pair_and_granularity(
                sym_val or "EURUSD", default_granularity=default_gran
            )
...
class GenericSimulatedTradingViewForexQuoteData(SimulatedEurUsdQuoteData):
    ...
    def get_source(
        ...
        default_gran = _granularity_from_increment(getattr(config, "increment", None))
        pair, gran = _extract_pair_and_granularity(
            getattr(config.symbol, "value", str(config.symbol)),
...
class HolidayData(PythonData):
    ...
    def reader(
        ...
        try:
            ...
            csv_date_str = dt.date().strftime(
                "%Y-%m-%d"
            )  # keep raw CSV date (no delta)
